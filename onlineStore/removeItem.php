...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['item_title'])) {
    ...
    $itemName = $_POST['item_title'];

    // Prepare the DELETE statement to remove the card from the user_cards table
    $stmt = $con->prepare('DELETE FROM shoppingcart WHERE user_id = ? AND item_title = ?');
    $stmt->bind_param('is', $loggedInUserId, $itemName);

    if ($stmt->execute()) {
        // Return a JSON response indicating success.
        echo json_encode(array('status' => 'success', 'message' => 'Item removed successfully!'));
    } else {
